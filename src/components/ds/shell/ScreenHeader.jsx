// Screen title in the pixel face, status line beneath it in Plex Mono, nav on
// the right. Press Start 2P is headings only — never more than four words.
export function ScreenHeader({ title, status, tone = "pink", nav }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap", marginBottom: 18 }}>
      ...
      {nav && <div style={{ display: "flex", gap: 6, alignItems: "flex-start" }}>{nav}</div>}
    </div>
  );
}
